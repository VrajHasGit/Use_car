...
const Documents = () => {
  ...
  const handleSave = async (fd) => {
    try {
      if (editRec) { await updateRecord('doc', editRec.id, fd); showToast('Updated!'); }
      else { const cnt = await getNextCounter('DOC'); await addRecord('doc', {...fd, docId: genId('DOC', cnt), date: fd.date||today()}); showToast('Documents record added!'); }
      await refresh('doc'); setIsModalOpen(false);
  ...
  return (
    <div className="page on" id="pg_documents">
      ...
      <div className="ph">
        <div className="ph-left"><h1><div className="ph-icon"><i className="fa fa-file-contract"></i></div>Documents</h1><p>Vehicle document tracking and management</p></div>
      ...
      <div className="tc">
        <div className="tc-hdr"><div className="tc-title">Documents <span style={{background:'var(--info)',color:'#fff',fontSize:10,fontWeight:700,padding:'2px 8px',borderRadius:10,marginLeft:8}}>{records.length}</span></div></div>
          ...
          {filtered.length === 0 ? (
            ...
            <table>
              ...
              <tbody>{filtered.map(r => (
                <tr key={r.id}>
                  <td style={{fontWeight:700,color:'var(--info)',fontFamily:"'Space Grotesk',sans-serif"}}>{r.docId||r.id?.slice(0,12)}</td>
