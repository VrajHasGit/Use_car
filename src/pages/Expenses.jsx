...
const Expenses = () => {
  ...
  const handleSave = async (fd) => {
    try {
      ...
      else { const cnt = await getNextCounter('EXP'); await addRecord('exp_rec', {...fd, expId: genId('EXP', cnt), date: fd.date||today()}); showToast('Expenses record added!'); }
      await refresh('exp_rec'); setIsModalOpen(false);
    } catch(e) { showToast('Failed: '+e.message, 'error'); }
  };
  const handleDelete = async (rec) => { if (!window.confirm('Delete?')) return; try { await deleteRecord('exp_rec', rec.id); await refresh('exp_rec'); showToast('Deleted.', 'info'); } catch(e) { showToast('Delete failed.', 'error'); } };
  return (
    <div className="page on" id="pg_expenses">
      ...
      <div className="ph">
        <div className="ph-left"><h1><div className="ph-icon"><i className="fa fa-receipt"></i></div>Expenses</h1><p>Track and manage dealership expenses</p></div>
      ...
      <div className="tc">
        <div className="tc-hdr"><div className="tc-title">Expenses <span style={{background:'var(--danger)',color:'#fff',fontSize:10,fontWeight:700,padding:'2px 8px',borderRadius:10,marginLeft:8}}>{records.length}</span></div></div>
        <div className="tbl-wrap">
          {filtered.length === 0 ? (
            <div className="empty" style={{padding:48}}>
              <i className="fa fa-receipt" style={{fontSize:36,color:'var(--border2)',display:'block',marginBottom:12}}></i>
              <div style={{fontSize:14,fontWeight:600,color:'var(--text2)',marginBottom:8}}>No records yet</div>
              <div style={{fontSize:12,color:'var(--text3)'}}>Click "Add Record" to create your first Expenses entry.</div>
            </div>
          ) : (
            <table>
              ...
              <tbody>{filtered.map(r => (
                <tr key={r.id}>
                  <td style={{fontWeight:700,color:'var(--danger)',fontFamily:"'Space Grotesk',sans-serif"}}>{r.expId||r.id?.slice(0,12)}</td>
