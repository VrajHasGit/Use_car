...
export const StkModal = ({ isOpen, onClose }) => {
 ...
 <div className="mbox"><div className="m-hdr"><div className="m-hdr-icon">🏭</div><h3>Car Stock</h3><button className="m-close" onClick={onClose} >✕</button></div>
 <div className="m-body">
  <div style={{"background":"rgba(255,107,0,.07)","border":"1px solid rgba(255,107,0,.25)","borderRadius":"var(--radius-sm)","padding":"10px 14px","marginBottom":"14px","display":"flex","alignItems":"center","gap":"10px"}}>
   ...
   <div className="fg" style={{"margin":"0","flex":"1"}}>
    ...
    <input id="sk_inqid" name="sk_inqid" value={formData['sk_inqid'] || ''} onChange={handleChange} placeholder="INQ-2025-0001 — type karo, sab fill ho jayega" style={{"background":"var(--bg)","border":"1px solid rgba(255,107,0,.4)","color":"var(--text)","borderRadius":"var(--radius-sm)","padding":"8px 12px","fontFamily":"inherit","fontSize":"12px","width":"100%"}}  />
   </div>
  </div>
  <div className="grid3"><div className="fg"><label>Registration No. * <span style={{"color":"var(--or1)","fontSize":"10px"}}>⚡ Auto-Fill by RegNo</span></label><input id="sk_regn" name="sk_regn" value={formData['sk_regn'] || ''} onChange={handleChange} placeholder="GJ-01-AB-1234"  /></div><div className="fg"><label>Chassis Number</label><input id="sk_chas" name="sk_chas" value={formData['sk_chas'] || ''} onChange={handleChange} placeholder="17-char VIN" /></div><div className="fg"><label>Engine Number</label><input id="sk_eng" name="sk_eng" value={formData['sk_eng'] || ''} onChange={handleChange} placeholder="Engine No." /></div></div>
  ...
  <div className="grid3"><div className="fg"><label>Purchase Price ₹ *</label><input type="number" id="sk_pp" name="sk_pp" value={formData['sk_pp'] || ''} onChange={handleChange} placeholder="0"  /></div><div className="fg"><label>Refurb Cost ₹</label><input type="number" id="sk_refurb" name="sk_refurb" value={formData['sk_refurb'] || ''} onChange={handleChange} placeholder="0"  /></div><div className="fg"><label>RTO Transfer Cost ₹</label><input type="number" id="sk_rto" name="sk_rto" value={formData['sk_rto'] || ''} onChange={handleChange} placeholder="0"  /></div></div>
  <div className="grid3"><div className="fg"><label>Insurance Cost ₹</label><input type="number" id="sk_ins" name="sk_ins" value={formData['sk_ins'] || ''} onChange={handleChange} placeholder="0"  /></div><div className="fg"><label>Total Cost Price ₹ (AUTO)</label><div className="calc-out" id="sk_tcp">₹ 0</div></div><div className="fg"><label>Sale Price ₹</label><input type="number" id="sk_sp" name="sk_sp" value={formData['sk_sp'] || ''} onChange={handleChange} placeholder="0"  /></div></div>
  <div className="grid3"><div className="fg"><label>2nd Price ₹ (Floor)</label><input type="number" id="sk_sp2" name="sk_sp2" value={formData['sk_sp2'] || ''} onChange={handleChange} placeholder="0" /></div><div className="fg"><label>3rd Price ₹ (Minimum)</label><input type="number" id="sk_sp3" name="sk_sp3" value={formData['sk_sp3'] || ''} onChange={handleChange} placeholder="0" /></div><div className="fg"><label>Profit / Loss ₹ (AUTO)</label><div className="calc-out" id="sk_profit">₹ 0</div></div></div>
  <div className="calc-panel">
   <div className="calc-row"><span className="cl">Purchase Price</span><span id="sks1">₹ 0</span></div>
   <div className="calc-row"><span className="cl">+ Refurbishment</span><span id="sks2">₹ 0</span></div>
   <div className="calc-row"><span className="cl">+ RTO Transfer</span><span id="sks3">₹ 0</span></div>
   <div className="calc-row"><span className="cl">+ Insurance</span><span id="sks4">₹ 0</span></div>
   <div className="calc-row"><span className="cl">= Total Cost</span><span id="sks5">₹ 0</span></div>
   <div className="calc-row"><span className="cl">Sale Price</span><span id="sks6">₹ 0</span></div>
   <div className="calc-row"><span style={{"color":"var(--or2)"}}>PROFIT / LOSS</span><span id="sks7" style={{"color":"var(--success)"}}>₹ 0</span></div>
  </div>
  <div className="grid3" style={{"marginTop":"14px"}}><div className="fg"><label>360° Video</label><select id="sk_360" name="sk_360" value={formData['sk_360'] || ''} onChange={handleChange}><option>No</option><option>Yes</option></select></div><div className="fg"><label>Listed on Portal</label><select id="sk_portal" name="sk_portal" value={formData['sk_portal'] || ''} onChange={handleChange}><option>No</option><option>OLX</option><option>CarDekho</option><option>Cars24</option><option>All</option></select></div><div className="fg"><label>Marketing</label><select id="sk_mkt" name="sk_mkt" value={formData['sk_mkt'] || ''} onChange={handleChange}><option>None</option><option>Social Media</option><option>WhatsApp</option><option>YouTube</option><option>All</option></select></div></div>
 </div>
