...
export const ValModal = ({ isOpen, onClose }) => {
 ...
 <div className="mbox"><div className="m-hdr"><div className="m-hdr-icon">🔍</div><h3>Vehicle Valuation</h3><button className="m-close" onClick={onClose} >✕</button></div>
 <div className="m-body">
  ...
  <div className="grid3"><div className="fg"><label>Status</label><select id="v_stat" name="v_stat" value={formData['v_stat'] || ''} onChange={handleChange}><option>Pending</option><option>Done</option><option>Approved</option><option>Rejected</option><option>Hold</option></select></div><div className="fg"><label>Next Follow-Up Date <span style={{"color":"var(--or1)","fontSize":"10px"}}>📅 PFU ma auto-set</span></label><input type="date" id="v_nextfu" name="v_nextfu" value={formData['v_nextfu'] || ''} onChange={handleChange} /></div><div className="fg"><label>Remarks</label><input id="v_rem" name="v_rem" value={formData['v_rem'] || ''} onChange={handleChange} placeholder="Notes" /></div></div>
  ...
  <div style={{"background":"var(--bg)","border":"1px solid var(--border2)","borderRadius":"var(--radius)","padding":"16px","marginBottom":"14px"}}>
   <div className="grid2" style={{"marginBottom":"12px"}}>
    <div className="fg">
     <label>📸 Vehicle Photos (Multiple)</label>
     <div className="upload-box" id="photoBox" >
      ...
      <span style={{"fontSize":"10px","color":"var(--text3)","marginTop":"3px","display":"block"}}>JPG, PNG — Multiple allowed</span>
      <input type="file" id="v_photos" accept="image/*" multiple style={{"display":"none"}}  />
     </div>
    </div>
    <div className="fg">
     <label>🎥 Vehicle Video (360° / Walk-around)</label>
     <div className="upload-box" id="videoBox" >
      ...
      <span style={{"fontSize":"10px","color":"var(--text3)","marginTop":"3px","display":"block"}}>MP4, MOV — Max 100MB</span>
      <input type="file" id="v_video" accept="video/*" style={{"display":"none"}}  />
     </div>
    </div>
   </div>
   <div id="photoPreview" style={{"display":"none","marginTop":"10px"}}>
    <div style={{"fontSize":"10px","fontWeight":"700","letterSpacing":".8px","textTransform":"uppercase","color":"var(--text3)","marginBottom":"8px"}}>📸 UPLOADED PHOTOS</div>
    <div id="photoGrid" style={{"display":"flex","flexWrap":"wrap","gap":"8px"}}></div>
   </div>
   <div id="videoPreview" style={{"display":"none","marginTop":"10px"}}>
    <div style={{"fontSize":"10px","fontWeight":"700","letterSpacing":".8px","textTransform":"uppercase","color":"var(--text3)","marginBottom":"8px"}}>🎥 UPLOADED VIDEO</div>
