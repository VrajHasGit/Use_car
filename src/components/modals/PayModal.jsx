...
export const PayModal = ({ isOpen, onClose }) => {
 ...
 <div className="mbox"><div className="m-hdr"><div className="m-hdr-icon">💳</div><h3>Payment Record</h3><button className="m-close" onClick={onClose} >✕</button></div>
 <div className="m-body">
  ...
  <div style={{"background":"rgba(255,107,0,.07)","border":"1px solid rgba(255,107,0,.25)","borderRadius":"var(--radius-sm)","padding":"10px 14px","marginBottom":"12px","display":"grid","gridTemplateColumns":"1fr 1fr","gap":"10px"}}>
   <div className="fg" style={{"margin":"0"}}>
    <label style={{"color":"var(--or3)","fontSize":"10px","fontWeight":"700","letterSpacing":".8px","textTransform":"uppercase","marginBottom":"4px","display":"block"}}>⚡ Order Booking ID (OB)</label>
   ...
   <div className="fg" style={{"margin":"0"}}>
    <label style={{"color":"#059669","fontSize":"10px","fontWeight":"700","letterSpacing":".8px","textTransform":"uppercase","marginBottom":"4px","display":"block"}}>⚡ Sales Order Booking ID (SOB)</label>
  ...
  <div className="grid3"><div className="fg"><label>Buyer Name</label><input id="py_name" name="py_name" value={formData['py_name'] || ''} onChange={handleChange} placeholder="Name" /></div><div className="fg"><label>Mobile</label><input id="py_contact" name="py_contact" value={formData['py_contact'] || ''} onChange={handleChange} placeholder="Contact No." readOnly /></div><div className="fg"><label>Vehicle Reg No. <span style={{"color":"var(--or1)","fontSize":"10px"}}>⚡ Auto</span></label><input id="py_regn" name="py_regn" value={formData['py_regn'] || ''} onChange={handleChange} placeholder="GJ-01-AB-1234" readOnly /></div></div>
  <div className="grid3"><div className="fg"><label>Car Details <span style={{"color":"var(--or1)","fontSize":"10px"}}>⚡ Auto</span></label><input id="py_mm" name="py_mm" value={formData['py_mm'] || ''} onChange={handleChange} placeholder="Make Model" readOnly /></div><div className="fg"><label>Finance Bank <span style={{"color":"var(--or1)","fontSize":"10px"}}>⚡ Auto</span></label><input id="py_finbank" name="py_finbank" value={formData['py_finbank'] || ''} onChange={handleChange} placeholder="Bank name (if financed)" readOnly /></div><div className="fg"><label>Finance Amount ₹ <span style={{"color":"var(--or1)","fontSize":"10px"}}>⚡ Auto</span></label><input type="number" id="py_finamt" name="py_finamt" value={formData['py_finamt'] || ''} onChange={handleChange} placeholder="0" readOnly /></div></div>
  ...
  <div className="grid3"><div className="fg"><label>Payment Amount ₹ *</label><input type="number" id="py_amt" name="py_amt" value={formData['py_amt'] || ''} onChange={handleChange} placeholder="0"  /></div><div className="fg"><label>Total Sale Amount ₹</label><input type="number" id="py_total" name="py_total" value={formData['py_total'] || ''} onChange={handleChange} placeholder="0"  /></div><div className="fg"><label>Previously Paid ₹</label><input type="number" id="py_prev" name="py_prev" value={formData['py_prev'] || ''} onChange={handleChange} placeholder="0"  /></div></div>
  <div className="calc-panel">
   <div className="calc-row"><span className="cl">Total Amount</span><span id="py_s1">₹ 0</span></div>
   <div className="calc-row"><span className="cl">Previously Paid</span><span id="py_s2">₹ 0</span></div>
   <div className="calc-row"><span className="cl">This Payment</span><span id="py_s3">₹ 0</span></div>
   <div className="calc-row"><span style={{"color":"var(--warn)"}}>BALANCE PENDING</span><span id="py_bal" style={{"color":"var(--warn)"}}>₹ 0</span></div>
  </div>
 ...
 <div className="m-foot"><button className="btn btn-out"  onClick={onClose}>Cancel</button><button className="btn btn-bl" ><i className="fa fa-file-contract"></i> વેચાણ ખત / Delivery Note</button><button className="btn btn-or" onClick={handleSave} ><i className="fa fa-save"></i> Record Payment</button></div></div>
</div>
  );
};
