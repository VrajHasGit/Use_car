...
export const WsModal = ({ isOpen, onClose }) => {
 ...
 <div className="mbox"><div className="m-hdr"><div className="m-hdr-icon">🔧</div><h3>Workshop / Refurbishment Job Card</h3><button className="m-close" onClick={onClose} >✕</button></div>
 <div className="m-body">
  ...
  <div className="grid1"><div className="fg"><label>Problem Details</label><textarea id="ws_prob" name="ws_prob" value={formData['ws_prob'] || ''} onChange={handleChange} placeholder="Customer complaint / problem description…"></textarea></div></div>
  <div className="grid1"><div className="fg"><label>Parts Required</label><textarea id="ws_parts" name="ws_parts" value={formData['ws_parts'] || ''} onChange={handleChange} placeholder="List all parts needed…"></textarea></div></div>
  <div className="sect-lbl"><i className="fa fa-calculator"></i> Cost Calculation — AUTO</div>
  <div className="grid3"><div className="fg"><label>Estimated Cost ₹</label><input type="number" id="ws_est" name="ws_est" value={formData['ws_est'] || ''} onChange={handleChange} placeholder="0" /></div><div className="fg"><label>Parts Cost ₹ *</label><input type="number" id="ws_pc" name="ws_pc" value={formData['ws_pc'] || ''} onChange={handleChange} placeholder="0"  /></div><div className="fg"><label>Labour Cost ₹ *</label><input type="number" id="ws_lc" name="ws_lc" value={formData['ws_lc'] || ''} onChange={handleChange} placeholder="0"  /></div></div>
  <div className="grid3"><div className="fg"><label>Total Cost ₹ (AUTO)</label><div className="calc-out" id="ws_total">₹ 0</div></div><div className="fg"><label>Payment Status</label><select id="ws_pstat" name="ws_pstat" value={formData['ws_pstat'] || ''} onChange={handleChange}><option>Pending</option><option>Paid</option></select></div><div className="fg"><label>Job Status</label><select id="ws_jstat" name="ws_jstat" value={formData['ws_jstat'] || ''} onChange={handleChange}><option>Open</option><option>In Process</option><option>Complete</option></select></div></div>
